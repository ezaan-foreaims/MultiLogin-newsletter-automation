...
def load_sites(file_path="/home/ezaan-amin/Documents/BitBash_Tasks/multi-login/failed_sites.txt"):
    """Load list of sites from a text file (one URL per line)."""
    if not os.path.exists(file_path):
        print(f"⚠️  sites.txt not found at '{file_path}', falling back to TEST_URL: {TEST_URL}")
        return [TEST_URL]
    sites = []
...
async def async_main(token, sites):
    """Async main entry point - now visits each site in 'sites' list."""
    ...
    try:
        for idx, site in enumerate(sites, start=1):
            print("\n" + "="*40)
            print(f"🔁 Visiting site {idx}/{len(sites)}: {site}")
            try:
                result = await run_main_flow(browser, site)
                success_overall[site] = bool(result)
            except Exception as e:
                print(f"❌ Error visiting {site}: {e}")
                success_overall[site] = False

            # Small randomized cool-down between visits to reduce captcha likelihood
            cooldown = random.uniform(6.0, 12.0)
            print(f"🕒 Waiting {cooldown:.1f}s before next site...")
            # some harmless mouse wiggle between visits (human-like)
            try:
                page = browser.contexts[0].pages[0] if browser.contexts and browser.contexts[0].pages else None
                if page:
                    await page.mouse.move(random.randint(50, 300), random.randint(50, 300), steps=6)
            except:
                pass
        ...
        print("\n📋 Visit summary:")
        for site, ok in success_overall.items():
            print(f" - {site} -> {'OK' if ok else 'FAILED'}")
